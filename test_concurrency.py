...
logging.basicConfig(filename='scp.log', level=logging.INFO,
...
class FileHandler(FileSystemEventHandler):
    ...
    def send_file(self, zip_file_path):
        try:
            ...
            if response.status_code == 200:
                logging.info(f"Successfully sent file {zip_file_path} to orthanc_studies, create patient and create patient report apis\n")
                ...
                logging.info(f"Failed to send file: {zip_file_path}, Status code: {response.status_code}, {response.json()}")
    ...
    def upload(self, orthanc_response):
        try:
            for i in range(0,len(orthanc_response)):
                ...
                if response.status_code == 200:
                    logging.info("Got the patient level information successfully from orthanc_studies api")
                    patient_info = response.json()['PatientMainDicomTags']
                    instance_id = response.json()['MainDicomTags']['StudyInstanceUID']
                    response_create_patient = self.create_patient(patient_info, response.json()['PatientMainDicomTags']['PatientID'])
                    pat_id = response_create_patient.get("patient_id")
                    self.create_patient_report(pat_id, instance_id, study_id)
                else:
                    logging.info(f"Failed to get the patient level information from orthanc_studies api, Status code: {response.status_code}, {response.json()}")
    ...
    def create_patient(self, patient_info, patient_id):
        ...
        try:
            patient_data = {
                ...
                "patient_mobile": "",
                "patient_email": "",
                "dob":datetime.datetime.strptime(date_string, f'%Y%m%d').date() if date_string!=''else None,
            ...
            response = requests.post('https://api.smaro.app/api/app/patient/create', json=patient_data, headers=headers, verify=False)
            ...
            if response.json()["statusCode"] in (200, 409):
                ...
                logging.info(f"Failed to upload create patient, Status code: {response.status_code}, {response.json()}\n")
    ...
    def create_patient_report(self, patient_id, instance_id, study_id):
        try:
            ...
            headers = {'Content-Type': 'application/json', 'token': self.token}
            response = requests.post('https://api.smaro.app/api/console/patient/report/create', json=report_data, headers=headers, verify=False)

            if response.json()["statusCode"] in (200, 409):
                logging.info(f"Response from create_patient_report api is {response.json()}\n")
                # patient_id = response.json()['data']['insertId']
                return {"success": True }
            else:
                logging.info(f"Failed to upload create patient report, Status code: {response.status_code}, {response.json()}\n")
